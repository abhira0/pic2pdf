...
class State:
    def __init__(self) -> None:
        ...
        dirPATH = ""
        pw = 0  # pic width
        w = 0  # window screen width
        ph = 0  # pic height
        h = 0  # window screen height
        filename = ""
        validity = 0
        DPI = 300  # default DPI
        imgchk = 0  # Image print flag
        pdfchk = 0  # Pdf print flag
        built = 0
        versionList = [["1.0", "07-01-2020", "Initial Version"],
                       ["1.1", "16-01-2020", "Flexibility for user has been added"]]
        copies = []


class BackEnd:
    def setState(self, given_state):
    ...
    def putBorder(self, pic):
        ...
        image_with_border = Image.new(
            "RGB",
            (
                (w + 2 * C4U.photo_borders["passport"][0]),
                (h + 2 * C4U.photo_borders["passport"][1]),
            ),
            (255, 255, 255, 255),
        )
        position = (C4U.photo_borders["passport"]
                    [0], C4U.photo_borders["passport"][1])
    ...
    def makefullcopies(self):
        paths = self.PATHS
        pw, ph = (int(self.pw * self.DPI / 25.4),
                  int(self.ph * self.DPI / 25.4))
        pw, ph = (
            (pw + 2 * C4U.photo_borders["passport"][0]),
            (ph + 2 * C4U.photo_borders["passport"][1]),
        )
        ...
        for j in li:
            ...
            pic = pic.resize(
                (int(self.pw * self.DPI / 25.4), int(self.ph * self.DPI / 25.4))
            )
    ...
    def validate(self, window, btn2, btn1, inp1, c1, c2, sub, validate, dpi):
        msg = []  # Warning/Error messages
        stateit = Frame(window)
        stateit.grid(column=0, row=8, pady=2, padx=C4U.grid_offsets["x1"])
        lab0 = Label(stateit)
        lab0["text"] = "Processing..."
        lab0.grid(
            ipadx=5, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"])
        # ----------------------------
        # image selection
        if len(self.PATHS) == 0:
        ...
        if (self.pw == 0) and (self.ph == 0):
            try:
                self.photo_type = btn2.get().split(":")[0]
                self.pw = C4U.photo_sizes[self.photo_type][0]
                self.ph = C4U.photo_sizes[self.photo_type][1]
            except:
        ...
        if len(msg) == 0:
            ...
            validate["state"] = tk.DISABLED
            sub["state"] = tk.NORMAL
        else:
            lab0["text"] = "Check again and validate"
            messagebox.showwarning(
                "Warnings",
                "\n".join(["{}> {}".format(i + 1, msg[i])
                           for i in range(len(msg))]),
            )
        lab0.grid_remove()
        stateit.destroy()

    def builder(self, window, validate, sub):
        ...
        if self.built == 0:
            ...
            self.built = 0
            messagebox.showinfo(
                "Job is done", "Successfully saved at the location :)")
...
class FrontEnd(BackEnd):
    ...
    def GUI(self):
        window = tk.Tk()
        window.title("All Size Photo Builder v{}".format(
            self.versionList[-1][0]))
        window.geometry(
            "{}x{}".format(
                C4U.grid_offsets["xscreen"], C4U.grid_offsets["yscreen"])
        )
        ...
        select_size.grid(
            column=0, row=2, pady=C4U.grid_offsets["y1"], padx=C4U.grid_offsets["x1"]
        )
        Separator(window).grid(column=0, row=3, sticky="ew")  # ? -----------
        saveit = Frame(window)
        saveit.grid(
            column=0, row=4, pady=C4U.grid_offsets["y1"], padx=C4U.grid_offsets["x1"]
        )
        Separator(window).grid(column=0, row=5, sticky="ew")  # ? -----------
        createit = Frame(window)
        createit.grid(
            column=0, row=6, pady=C4U.grid_offsets["y1"], padx=C4U.grid_offsets["x1"]
        )
        Separator(window).grid(column=0, row=9, sticky="ew")  # ? -----------
        noteit = Frame(window)
        noteit.grid(
            column=0, row=10, pady=C4U.grid_offsets["y1"], padx=C4U.grid_offsets["x1"]
        )
        # ---------------------------------------------------------------------------
        # Select images [frame]
        # ---------
        Label(select_images, text="Please select the image(s): ").grid(
            column=0, row=1, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        btn3 = Button(select_images, text="Select...", command=self.selImgBtn)
        btn3.grid(
            column=1, row=1, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        btn4 = Button(select_images, text="See selected images",
                      command=self.seeSelImg)
        btn4.grid(
            columnspan=2, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        # ---------
        # ---------------------------------------------------------------------------
        # Select size [frame]
        # ---------
        # --------- photo size
        Label(select_size, text="Given photo type[ratio]: ").grid(
            column=0, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        ...
        btn2["values"] = val2
        btn2.grid(
            column=1, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        # ---------
        # -------- custom photo size
        ...
        btncstm = Button(
            cusFra2, text="Custom", command=lambda: self.customsize(cusFra2, 1, btn2)
        )
        btncstm.grid(
            column=0,
            row=0,
        ...
        # ---------
        # --------O/p size
        Label(select_size, text="Required output paper size: ").grid(
            column=0, row=3, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        ...
        btn1["values"] = val1
        btn1.grid(
            column=1, row=3, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        # ---------
        # --------custom o/p size
        ...
        btncstm2 = Button(
            cusFra, text="Custom", command=lambda: self.customsize2(cusFra, 0, btn1)
        )
        btncstm2.grid(
            column=0,
            row=0,
        ...
        # Save it [frame]
        # --------- filename input
        Label(saveit, text="Choose a filename to save: ").grid(
            column=0, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        ...
        Label(saveit, text="What do you want: ").grid(
            column=0, row=1, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        ...
        # --------- where to save
        Label(saveit, text="Choose a directory to save: ").grid(
            column=0, row=2, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        btn5 = Button(
            saveit, text="Choose...", command=lambda: self.chooseopdir(saveit)
        )
        btn5.grid(
            column=1, row=2, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        # ---------
        # ---------------------------------------------------------------------------
        ...
        validate = Button(
            ...
            command=lambda: self.validate(
                window, btn2, btn1, inp1, c1, c2, sub, validate, dpi
            ),
        )
        validate.grid(
            ipady=5,
        ...
        sub = Button(
            ...
            text="Build",
            state="disabled",
            command=lambda: self.builder(window, validate, sub),
        )
        sub.grid(
            column=1,
            ipady=5,
        ...
        refr = Button(
            ...
            text="Refresh",
            command=lambda: (
                self.refreshwindow(btn1, btn2, inp1, c1,
                                   c2, validate, sub, dpi)
            ),
        )
        refr.grid(
        ...
        Label(noteit, text=note1).grid(
            column=0, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        note2 = "Built by ARG-Z"
        Label(noteit, text=note2).grid(
            column=0, row=1, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        note2 = "Updated on {} [version {}]".format(
            self.versionList[-1][1], self.versionList[-1][0])
        Label(noteit, text=note2).grid(
            column=0, row=3, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        # ---------
    ...
    def chooseopdir(self, saveit):
        diPATH = filedialog.askdirectory(
            initialdir=os.getcwd(), title="Select image(s)"
        )
        self.dirPATH = diPATH
        if self.dirPATH == "":
            messagebox.showwarning(
                title="Warning @ choosing",
                message="Output directory path cannot be empty",
            )
    ...
    def customsize(self, cusfra, i, comboB):
        ...
        self.bu.grid(
            column=5, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )

        self.xxx.grid(
            column=2, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        self.yyy.grid(
            column=3, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
        self.su = Button(
            cusfra,
            text="Save",
            width=5,
            command=lambda: self.savecuslen(
                i, self.xxx, self.yyy, self.su, comboB),
        )
        self.su.grid(
            column=4, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
    ...
    def customsize2(self, cusfra, i, comboB):
        ...
        self.su2 = Button(
            ...
            width=5,
            command=lambda: self.savecuslen(
                i, self.xxx2, self.yyy2, self.su2, comboB),
        )
        self.su2.grid(
            column=4, row=0, pady=C4U.grid_offsets["y2"], padx=C4U.grid_offsets["x2"]
        )
    ...
    def savecuslen(self, i, x, y, su, comboB):
        try:
            ...
            if int(x.get()) <= 0 or int(y.get()) <= 0:
                messagebox.showwarning(
                    "Zero is unacceptable", "Enter the numbers(integers/decimals)"
                )
            else:
            ...
            messagebox.showwarning(
                "Not a number", "Enter the numbers(integers/decimals)"
            )
    ...
    def refreshwindow(self, btn1, btn2, inp1, c1, c2, validate, sub, dpi):
        self.PATHS = []
        self.dirPATH = ""
        self.pw = 0
        self.w = 0
